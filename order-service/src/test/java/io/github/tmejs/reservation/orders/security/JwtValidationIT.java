...
class JwtValidationIT {
    ...
    void acceptsValidToken() throws Exception {
        mockMvc.perform(get("/orders/validation").header("Authorization", "Bearer " + token(TokenClaims.valid(), SIGNING_KEY)))
                .andExpect(status().isOk());
    }

    @Test
    void rejectsTokenSignedByDifferentKey() throws Exception {
        mockMvc.perform(get("/orders/validation").header("Authorization", "Bearer " + token(TokenClaims.valid(), WRONG_KEY)))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void rejectsWrongIssuer() throws Exception {
        mockMvc.perform(get("/orders/validation").header("Authorization", "Bearer "
    ...
    void rejectsWrongAudience() throws Exception {
        mockMvc.perform(get("/orders/validation").header("Authorization", "Bearer "
                        + token(TokenClaims.valid().withAudience(List.of("inventory-api")), SIGNING_KEY)))
                .andExpect(status().isUnauthorized());
    }
