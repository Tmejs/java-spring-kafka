...
final class ProductProblemHandler {

    @ExceptionHandler(ProductNotFoundException.class)
    ResponseEntity<ProblemDetail> notFound(ProductNotFoundException exception, HttpServletRequest request) {
        return problem(HttpStatus.NOT_FOUND, "Product not found", exception.getMessage(), request, List.of());
    }

    @ExceptionHandler(StockOverflowException.class)
    ResponseEntity<ProblemDetail> conflict(StockOverflowException exception, HttpServletRequest request) {
        return problem(HttpStatus.CONFLICT, "Stock quantity conflict", exception.getMessage(), request, List.of());
    }
    ...
    ResponseEntity<ProblemDetail> constraintViolation(
        ...
        List<Map<String, String>> errors = exception.getConstraintViolations().stream()
                .map(ProductProblemHandler::toFieldError)
                .toList();
        return problem(HttpStatus.BAD_REQUEST, "Invalid request", "Request validation failed", request, errors);
    }
